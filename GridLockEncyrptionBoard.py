test_unique_key = "Gridlock_filler_key"
...
def charto32bitarray(instring):
    temparr= []
    x = 0
    for i, x in enumerate(instring):
        temparr.append(x.encode('utf-32')) #translate each character into 1 32bit value
    return temparr
flatarr = charto32bitarray(test_info) #make a single flat array of the encoded bytes
print(flatarr)
...
print("testing multiplication (multiplying by 300)")
print(mul_byte(flatarr[0],300))
print(byteTo_char(mul_byte(flatarr[0],300)))
